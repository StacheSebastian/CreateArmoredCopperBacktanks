...
public class IronBacktankRenderer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {

    public static final IIngameOverlay REMAINING_AIR_OVERLAY = IronBacktankRenderer::renderRemainingAirOverlay;

    public IronBacktankRenderer(RenderLayerParent<T, M> renderer) {
        super(renderer);
    }
    ...
    public void render(PoseStack ms, MultiBufferSource buffer, int light, LivingEntity entity, float yaw, float pitch,
        ...
        if (entity.getPose() == Pose.SLEEPING)
            return;
        if (!ModItems.IRON_REINFORCED_COPPER_BACKTANK.get()
                .isWornBy(entity))
            return;
        ...
        BlockState renderedState = ModBlocks.IRON_REINFORCED_COPPER_BACKTANK.getDefaultState()
                .setValue(IronReinforcedCopperBacktankBlock.HORIZONTAL_FACING, Direction.SOUTH);
    ...
    public static void registerOn(EntityRenderer<?> entityRenderer) {
        ...
        if (!(livingRenderer.getModel() instanceof HumanoidModel))
            return;
        IronBacktankRenderer<?, ?> layer = new IronBacktankRenderer<>(livingRenderer);
        livingRenderer.addLayer((IronBacktankRenderer) layer);
    }
    ...
    public static void renderRemainingAirOverlay(ForgeIngameGui gui, PoseStack poseStack, float partialTicks, int width, int height) {
        ...
        GuiGameElement.of(ModItems.IRON_REINFORCED_COPPER_BACKTANK.asStack())
                .at(0, 0)
                .render(poseStack);
