...
import io.github.stachesebastian.createab.register.backtanks.diamond.DiamondReinforcedCopperBacktankBlock;
...
public class DiamondBacktankRenderer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {

    public static final IIngameOverlay REMAINING_AIR_OVERLAY = DiamondBacktankRenderer::renderRemainingAirOverlay;

    public DiamondBacktankRenderer(RenderLayerParent<T, M> renderer) {
        super(renderer);
    }
    ...
    public void render(PoseStack ms, MultiBufferSource buffer, int light, LivingEntity entity, float yaw, float pitch,
        ...
        if (entity.getPose() == Pose.SLEEPING)
            return;
        if (!ModItems.DIAMOND_REINFORCED_COPPER_BACKTANK.get()
                .isWornBy(entity))
            return;
        ...
        BlockState renderedState = ModBlocks.DIAMOND_REINFORCED_COPPER_BACKTANK.getDefaultState()
                .setValue(DiamondReinforcedCopperBacktankBlock.HORIZONTAL_FACING, Direction.SOUTH);
    ...
    public static void registerOn(EntityRenderer<?> entityRenderer) {
        ...
        if (!(livingRenderer.getModel() instanceof HumanoidModel))
            return;
        DiamondBacktankRenderer<?, ?> layer = new DiamondBacktankRenderer<>(livingRenderer);
        livingRenderer.addLayer((DiamondBacktankRenderer) layer);
    }
    ...
    public static void renderRemainingAirOverlay(ForgeIngameGui gui, PoseStack poseStack, float partialTicks, int width, int height) {
        ...
        GuiGameElement.of(ModItems.DIAMOND_REINFORCED_COPPER_BACKTANK.asStack())
                .at(0, 0)
                .render(poseStack);
