package io.github.stachesebastian.createab.register.render;
...
import io.github.stachesebastian.createab.register.ModBlocks;
import io.github.stachesebastian.createab.register.ModItems;
import io.github.stachesebastian.createab.register.backtanks.netherite.NetheriteReinforcedCopperBacktankBlock;
...
public class NetheriteBacktankRenderer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {

    public static final IIngameOverlay REMAINING_AIR_OVERLAY = NetheriteBacktankRenderer::renderRemainingAirOverlay;

    public NetheriteBacktankRenderer(RenderLayerParent<T, M> renderer) {
        super(renderer);
    }
    ...
    public void render(PoseStack ms, MultiBufferSource buffer, int light, LivingEntity entity, float yaw, float pitch,
        ...
        if (entity.getPose() == Pose.SLEEPING)
            return;
        if (!ModItems.NETHERITE_REINFORCED_COPPER_BACKTANK.get()
                .isWornBy(entity))
            return;
        ...
        BlockState renderedState = ModBlocks.NETHERITE_REINFORCED_COPPER_BACKTANK.getDefaultState()
                .setValue(NetheriteReinforcedCopperBacktankBlock.HORIZONTAL_FACING, Direction.SOUTH);
    ...
    public static void registerOn(EntityRenderer<?> entityRenderer) {
        ...
        if (!(livingRenderer.getModel() instanceof HumanoidModel))
            return;
        NetheriteBacktankRenderer<?, ?> layer = new NetheriteBacktankRenderer<>(livingRenderer);
        livingRenderer.addLayer((NetheriteBacktankRenderer) layer);
    }
    ...
    public static void renderRemainingAirOverlay(ForgeIngameGui gui, PoseStack poseStack, float partialTicks, int width, int height) {
        ...
        GuiGameElement.of(ModItems.NETHERITE_REINFORCED_COPPER_BACKTANK.asStack())
                .at(0, 0)
                .render(poseStack);
