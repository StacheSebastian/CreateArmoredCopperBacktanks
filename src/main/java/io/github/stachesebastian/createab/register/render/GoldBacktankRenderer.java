...
import io.github.stachesebastian.createab.register.backtanks.gold.GoldReinforcedCopperBacktankBlock;
...
public class GoldBacktankRenderer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {

    public static final IIngameOverlay REMAINING_AIR_OVERLAY = GoldBacktankRenderer::renderRemainingAirOverlay;

    public GoldBacktankRenderer(RenderLayerParent<T, M> renderer) {
        super(renderer);
    }
    ...
    public void render(PoseStack ms, MultiBufferSource buffer, int light, LivingEntity entity, float yaw, float pitch,
        ...
        if (entity.getPose() == Pose.SLEEPING)
            return;
        if (!ModItems.GOLD_REINFORCED_COPPER_BACKTANK.get()
                .isWornBy(entity))
            return;
        ...
        BlockState renderedState = ModBlocks.GOLD_REINFORCED_COPPER_BACKTANK.getDefaultState()
                .setValue(GoldReinforcedCopperBacktankBlock.HORIZONTAL_FACING, Direction.SOUTH);
    ...
    public static void registerOn(EntityRenderer<?> entityRenderer) {
        ...
        if (!(livingRenderer.getModel() instanceof HumanoidModel))
            return;
        GoldBacktankRenderer<?, ?> layer = new GoldBacktankRenderer<>(livingRenderer);
        livingRenderer.addLayer((GoldBacktankRenderer) layer);
    }
    ...
    public static void renderRemainingAirOverlay(ForgeIngameGui gui, PoseStack poseStack, float partialTicks, int width, int height) {
        ...
        GuiGameElement.of(ModItems.GOLD_REINFORCED_COPPER_BACKTANK.asStack())
                .at(0, 0)
                .render(poseStack);
